...
#----------------------------------------------------------------------------------------------------------------------#
def Spacer():
    '''Inserts a text spacer in the dialog window'''
    txt.insert(END, "-"*134)
    txt.insert(END, "\n")
    txt.update()
    txt.see("end")
...
def Create_Output_Folder(new_folder_name):
    '''Creates a new folder with the input name. If already present, skips.'''
    try:
...
def Find_Experiments(path_filename):
    '''Divides txt file into individual experiments. Saves new txt files in new folder with the name of the original txt file'''
...
def Divide_Curve_And_Plot(path_filename):
    '''Divides a file into loading and unloading data, and creates a plot and corresponding csv file for it.'''
    path, filename = os.path.split(path_filename)
    filename_noext, extension = os.path.splitext(filename)
...
def Unit_Transform(df,R):
    '''Transform R to m and F to Pa. Returns locations of several other columns'''
    R = R*1e-6
...
def Contact_Point(df, PosZ_um_col, PosZ_m_col, PiezoZ_m_col,Displacement_m_col, ratio_BL_points, threshold_constant):
    '''Determines Z actuation type, probe displacement, and contact point. Returns Zmov_type, Probe_Displacement_m_col, forward_scan, back_scan, forward_scan_index_min, forward_scan_min, back_scan_index_min, back_scan_min'''
    delta_stepmotor = df.iloc[2, PosZ_um_col] - df.iloc[3, PosZ_um_col]
...
def Indentation_Speed(Zmov_type, forward_scan):
    '''Determines indentation speed from Zmov_type and forward_scan data. Returns indentation_speed'''
...
def Baseline_And_Zero(df,forward_scan,forward_scan_index_min,Displacement_m_col,ratio_BL_points):
    '''Determines a linear baseline from first part of curve and subtracts from the data. returns baseline corrected data'''
...
def Max_Indent_And_Max_Force(forward_scan, Indentation_m_col, CorrForceA_N_col):
    '''Gets Max_Force and Max_Indentation based on last value of the two corresponding columns'''
...
def GetXY(df, Zmov_type):
    '''Obtains the X and Y coordinates from the corresponding data columns'''
    if Zmov_type == "Piezo scanner":
...
def Hertz_Fitting(df, forward_scan_segment, back_scan_segment, R, forward_scan_index_min, back_scan_index_min, Indentation_m_col, CorrForceA_N_col):
    '''Fits data to Hertz model (using algorithm and full curve fitting), returns optimal fitting parameters for both methods'''
    def Hertz(R, K, x):
        '''Equation for Hertz model'''
        a = ((R / K) * (x)) ** (1 / 3)
        return ((a ** 2) / R)
...
def Hertz_Print_Results(path_filename_noext, folder_path_fitting_restults, filename_noext, df, R, v_sample, Zmov_type, forward_scan, back_scan, forward_scan_contactP, back_scan_contactP, forward_scan_cov_Hertz_best, forward_scan_pars_Hertz_best, back_scan_cov_Hertz_best, back_scan_pars_Hertz_best, indentation_speed, Max_Indent, Max_Force, forward_scan_bulk_modulus_Hertz_full_curve, forward_scan_bulk_modulus_error_Hertz_full_curve, back_scan_bulk_modulus_Hertz_full_curve, back_scan_bulk_modulus_error_Hertz_full_curve, time_s):
    '''Prints experiment information in dialog window. Calculates Young's moduli for both fitting methods and prints to dialog window. Saves all data as a csv. Returns calculated outputs.'''
...
def Hertz_Analyse_Single_CSVFile(path_filename, folder_path_fitting_restults, filename_noext, ratio_BL_points, threshold_constant, forward_scan_segment, back_scan_segment, R, v_sample):
    '''Analyses a single csv file: uses linked functions to transform units, determine scan speed and contact point, subtract a baseline and zero data, and fit data to chosen model. Collects all model outputs and saves figures showing the model fit. Returns all model output for 2D mapping functions'''
...
def DMT_Fitting(df, forward_scan_segment, back_scan_segment, R, forward_scan_index_min, back_scan_index_min, Indentation_m_col, CorrForceA_N_col):
    '''Fits data to DMT model (using algorithm and full curve fitting), returns optimal fitting parameters for both methods'''
    #Define fitting functions
    def DMT(x, R, K, Fad):
        '''Equation for DMT fitting'''
        a = ((R/K)*(x+Fad))**(1/3)
        return ((a**2)/R)
...
def PT_Fitting(df, forward_scan_segment, back_scan_segment, R, forward_scan_index_min, back_scan_index_min, Indentation_m_col, CorrForceA_N_col):
    '''Fits data to PT model (using algorithm and full curve fitting), returns optimal fitting parameters for both methods'''
    #Define fitting functions
...
def JKR_Fitting(df, forward_scan_segment, back_scan_segment, R, forward_scan_index_min, back_scan_index_min, Indentation_m_col, CorrForceA_N_col):
    '''Fits data to JKR model (using algorithm and full curve fitting), returns optimal fitting parameters for both methods'''
    #Define fitting functions
    def JKR(x, R, K, Fad, dc):
        '''Equation for JKR fitting'''
...
def Divide_Array_And_Plot(path_filename):
    '''Takes a single txt file containing an array of measurements, divides into individual measurements. FOr each measurement, a plot and a csv file of the data are saved'''
    path, filename = os.path.split(path_filename)
    filename_noext, extension = os.path.splitext(filename)
...
def Generate3DPlot(data_list, folder_path_fitting_restults, filename_element, Z_axis_label, Z_axis): #Takes in input a 2D list [[Exp_number, X, Y, Z], [Exp_number, X, Y, Z], ...]
    '''Take a 2D list [[Exp_number, X, Y, Z] input, saves data as csv, then attempts to plot data as 2D and 3D plots (may have issues with missing data)'''
...
def Find_Diplacement_For_Threshold_Map(path_filename, ratio_BL_points, thres_F):
    '''Baselines data, determines contact point, and determines diplacement difference between contact point and threshold force value'''
    #Open df
    path, filename = os.path.split(path_filename)
    df = pandas.read_table(path_filename, sep=',', header=0)
    df = df.astype(float)  #Change data from object to float
...
def File_Extractor_b1():
    '''File extractor button. Takes a folder, saves a copy of original data in a new folder, then searches for data.txt files and saves them as their parent folder name'''
...
def Gen_Single_Exp_b2():
    '''Find curves button. Takes a single txt file with many indentation curves and divides into individual experiment txt files'''
...
def Divide_And_Plot_All_Exp_b3():
    '''Txt to csv and plot curves button. Takes a folder of txt files and runs Divide_Curve_And_Plot function on each file. Returns plots and csv files for all measurements'''
    #SET GLOBAL VARIABLE TO BE USED IN OTHER BUTTONS
    global path_filename #Global to reassing value adn have it available outside the button's function.
    global path
    global filename
...
def FitCurves():
    '''Fit curves! button. Select a number of csv files to batch process them by fitting to chosen model'''
...
def Divide_Array_And_Plot_All_Exp_b4():
    '''Txt to csv and plot curves button. Takes a folder a txt file containing an array of measurements divides and plots the individual measurements. Returns plots and csv files for all measurements'''
    #SET GLOBAL VARIABLE TO BE USED IN OTHER BUTTONS
    global path_filename #Global to reassing value adn have it available outside the button's function.
    global path
    global filename
...
def Generate3DMaps_b5():
    '''Generate 3D maps! button. Select a number of csv files to batch process them by fitting to chosen model and plot maps or the calculated mechanical properties'''
    txt.delete(1.0, END) #Clear dialog window
...
def ThresholdMap_b6():
    '''Threshold map! button. Select a number of csv files to batch process them by determining CP-force threhshold distance for each file and plotting data as a map'''
    txt.delete(1.0, END) #Clear dialog window
...
def SaveReport_b7():
    '''Saves a copy of the dialog window as a txt file'''
